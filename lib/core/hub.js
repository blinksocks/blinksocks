...
class Hub {
  ...
  constructor(config) {
    ...
    this._onConnection = (conn, proxyRequest = null) => {
      ...
      _utils.logger.verbose(`[hub] [${sourceAddress.host}:${sourceAddress.port}] connected`);
  ...
  async _createServerOnServer() {
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (local_protocol) {
        ...
        case 'h2':
          {
            server = _http4.default.createSecureServer({ key: tls_key, cert: tls_cert });
