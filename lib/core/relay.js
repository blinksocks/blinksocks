...
class Relay extends _events2.default {
  ...
  constructor({ config, source, transport, presets = [] }) {
    ...
    this._pipe = new _pipe.Pipe({ config, presets, isUdp: transport === 'udp' });
    ...
    if (config.is_server && config.acl) {
      this._acl = new _acl.ACL({ sourceAddress: this._source, rules: config.acl_rules });
