...
const randomBytesPool = _crypto2.default.randomBytes(RANDOM_BYTES_POOL_SIZE);

let randptr = 0;

function _refillRandomBytesPool() {
  randomBytesPool.fill(_crypto2.default.randomBytes(RANDOM_BYTES_POOL_SIZE));
  randptr = 0;
}
...
function randomBytes(len) {
  ...
  if (end < RANDOM_BYTES_POOL_SIZE) {
    ...
    const extra = _crypto2.default.randomBytes(end - RANDOM_BYTES_POOL_SIZE);
...
function hash(algorithm, buffer) {
  const hs = _crypto2.default.createHash(algorithm);
  hs.update(buffer);
  return hs.digest();
}

function hmac(algorithm, key, buffer) {
  const hmac = _crypto2.default.createHmac(algorithm, key);
  ...
  return {
    nextBytes: function nextBytes(n) {
      if (iter + n > buffered.length) {
        const hash = _jsSha2.default.shake128.create(buffered.length * 8 + 512);
        hash.update(buffer);
