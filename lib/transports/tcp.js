...
const MAX_BUFFERED_SIZE = 512 * 1024;class TcpInbound extends _defs.Inbound {

  constructor(props) {
    super(props);
    this._socket = null;
    this._speeder = new _utils.SpeedTester();
    ...
    this.onReceive = buffer => {
      ...
      if (outbound && outbound.bufferSize > MAX_BUFFERED_SIZE) {
        ...
        _utils.logger.debug(`[${this.name}] [${this.remote}] recv paused for ${sec}s due to inbound.bufferSize=${outbound.bufferSize} >= ${MAX_BUFFERED_SIZE}`);
        this.pause();
        setTimeout(() => {
...
class TcpOutbound extends _defs.Outbound {
  constructor(...args) {
    ...
    return _temp = super(...args), this._socket = null, this._speeder = new _utils.SpeedTester(), this._destroyed = false, this.onError = err => {
      _utils.logger.warn(`[${this.name}] [${this.remote}] ${err.message}`);
      this.emit('_error', err);
    }, this.onReceive = buffer => {
      this.emit('data', buffer);
      ...
      if (inbound && inbound.bufferSize > MAX_BUFFERED_SIZE) {
        ...
        const sec = (delta / speed).toFixed(3);
        _utils.logger.debug(`[${this.name}] [${this.remote}] recv paused for ${sec}s due to inbound.bufferSize=${inbound.bufferSize} >= ${MAX_BUFFERED_SIZE}`);
        this.pause();
        setTimeout(() => {
          _utils.logger.debug(`[${this.name}] [${this.remote}] resume to recv`);
          this.resume();
  ...
  async _connect({ host, port }) {
    ...
    _utils.logger.info(`[${this.name}] [${this.remote}] connecting to tcp://${host}:${port}` + (_net2.default.isIP(host) ? '' : ` resolved=${ip}`));
